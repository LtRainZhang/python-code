...
for infor in traj:
    ...
    bbbb = tot_ele[:, 3:4]
    zuizhong = np.concatenate((aaaa, bbbb, np.zeros(shape=(len(aaaa), 1))), axis=1)
    ...
    for element in zuizhong:
        if element[3] == 3:
            ...
            zuizhong[hh, 4] = distance
        hh = hh + 1

    for r in np.linspace(0.05,8,160):

        for i,element in enumerate(zuizhong):
            if element[3] == 3:
                ele_select = element
                distance = element[4]
                if distance > (r - thickness/2) and distance < (r + thickness/2):
                    jishu = jishu + 1
        density.append((jishu * 18/ (6.022 * math.pow(10, 23)) / (4 / 3 * 3.1415926 * (6*r*r*(thickness/2)*math.pow(10, -24)+2*math.pow(thickness/2,3)*math.pow(10, -24)))))
