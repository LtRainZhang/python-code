...
def getmsd(data1, tstep):
    xup, xdn = msdxy(data1, 2500) #迭代最大值
...
timestep = 1 #1=1ps
molist = [1, 20000, 225, 225, 32, 100, 20, 56, 46, 36, 140, 80, 64, 20, 20, 10, 14, 14, 4, 24, 18]
molsum = [sum(molist[:ix+1]) for ix in range(len(molist))]
molsum.append(0)
allmsd = []
sqr_dis = []
mass = [0, 18, 23, 35.5, 170, 226, 282, 226, 282, 196, 196, 176, 212, 202, 202, 167, 223, 217, 186, 170, 184]
for ix in range(len(molsum)-1):  #for ix in range(5, len(molsum)-1):  从第五个分子开始算
    msddata = getmsd(datas[:, molsum[ix-1]:molsum[ix], 0], timestep) #0=x,1=y,2=z
